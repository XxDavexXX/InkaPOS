...
class _PedidoState extends State<Pedido> {
  ...
  Map _getData({required String tipo}) {
    ...
    return {
      ...
      'vendedor':
          widget.vendedor ??
          {'nombre': getUsuario(getTurnoActual()!['usuarioID'])!['usuario']},
      'productos': _items,
      'igv': _getAverageIGV(),
      'qr': 'texto del qr',
  ...
  String _getAverageIGV() {
    ...
    bool allItemsHaveTheSameIGV =
        _items.map((prod) => prod['igv']).toList().toSet().toList().length == 1;
  ...
  void _editUnits(Map item) async {
    ...
    if (newUnits < 1) {
      alert(context, 'Cantidad no válida');
      return;
    }
    Map map = {...item};
    ...
    try {
      ...
      await alert(context, 'Unidades no válidas');
    } finally {
      Navigator.pop(context);
    }
  }
  ...
  void _deleteOrder() async {
    ...
    if (reason == null || reason.trim() == '') {
      alert(context, 'Ingrese un motivo válido');
      return;
    }
    doLoad(context);
    try {
  ...
  void _onItemTap(Map item) async {
    List<String> opts = ['Editar observaciones', 'Editar precio'];
    if (item['observacion'] != null) opts.add('Eliminar observaciones');
    if (item['descuento'] != null || item['nuevoPrecio'] != null)
      opts.add('Devolver a precio original');
    int? opt = await choose(context, opts, text: item['nombre']);
    if (opt == null) return;
    switch (opts[opt!]) {
      case 'Editar observaciones':
        _editItemObservationsDialog(item);
        break;
      case 'Editar precio':
        _editItemPriceDialog(item);
        break;
      case 'Eliminar observaciones':
        _eliminarObservaciones(item);
        break;
      case 'Devolver a precio original':
        _devolverPrecioOriginal(item);
        break;
    }
  }

  void _eliminarObservaciones(Map item) =>
      setState(() => item.remove('observacion'));
  void _devolverPrecioOriginal(Map item) =>
      setState(() => item.remove('nuevoPrecio'));
    ...
    String? newObservation = await dialog(
      ...
      children: [
        ...
        Div(
          ...
          width: width(context) * 0.9,
          height: 47,
          child: Center(child: Input(controller, hint: 'Observaciones')),
        ),
        sep,
        ...(item['observaciones'] ?? <String>[]).map<Widget>(
          (String ob) => Center(
            child: SizedBox(
              width: width(context) * 0.9,
              child: Button(
        ...
        sep,
        Button(
          P('Aceptar', color: Colors.white),
          () => Navigator.pop(context, controller.text.trim()),
        ),
        sep,
        Button(
          P('Cancelar', color: Colors.white),
          () => Navigator.pop(context),
        ),
      ],
    );
  ...
  void _editItemPriceDialog(Map item) async {
    ...
    switch (opts[opt!]) {
      case 'Descuento':
        _addDiscountToItem(item);
        break;
      case 'Cambio de precio':
        _changePriceOfItem(item);
        break;
    }
  }

  void _addDiscountToItem(Map item) async {
    if ((item['nuevoPrecio'] != null) &&
        (await confirm(
              context,
              'Al agregar un descuento, estarías eliminando el cambio de precio antes hecho en este producto, ya que el descuento se hará en base al precio original. ¿Eliminar el cambio de precio para proseguir con agregar descuento?',
            )) !=
            true)
      return;
    ...
    if (0 > x || x > 100) {
      alert(context, 'El % debe estar entre 0 y 100');
      return;
    }
    setState(() {
  ...
  void _changePriceOfItem(Map item) async {
    if ((item['descuento'] != null) &&
        (await confirm(
              context,
              'Al cambiar el precio, estarías borrando el descuento agregado a este producto, ¿Eliminar el descuento agregado previamente para proseguir con el cambio de precio?',
            )) !=
            true)
      return;
    ...
    if (x < 0) {
      alert(context, 'El precio no puede ser menor a 0');
      return;
    }
    setState(() {
  ...
  void _boleta() {
    goTo(
      context,
      TotalAPagar(datos: _getData(tipo: 'boleta'), vendedor: widget.vendedor),
    );
  }
  ...
  void _factura() async {
    if (_cliente == null) {
      ...
      Map? selectedClient = await goTo(
        context,
        const Clientes(selectClient: true),
      );
    ...
    goTo(
      context,
      TotalAPagar(datos: _getData(tipo: 'factura'), vendedor: widget.vendedor),
    );
  }
          ...
          builder:
              (context, setState) => SimpleDialog(
                ...
                children: [
                  ...
                  if (!showButton)
                    Center(child: Button(P('Configuraciones'), () {})),
                  sep,
                  GestureDetector(
  ...
  void doneSnackBar(String text) => showSnackBar(
    context,
    '¡Listo!',
    text,
    seconds: 1,
  ...
  void _removerTodasLasObservaciones() async {
    if ((await confirm(
          context,
          '¿Remover todas las observaciones de los productos?',
        )) !=
        true)
      return;
  ...
  void _removerTodosLosDescuentos() async {
    if ((await confirm(
          context,
          '¿Eliminar todos los descuentos de los productos?',
        )) !=
        true)
      return;
  ...
  void _removerTodosLosCambiosDePrecio() async {
    if ((await confirm(context, '¿Remover todos los cambios de los precio?')) !=
        true)
      return;
  ...
  void _definirIgvGeneral() async {
    if ((await confirm(
          context,
          '¿Definir un IGV general para todos los productos?',
        )) !=
        true)
      return;
    String? igvString = await prompt(
      context,
      text: 'IGV general (%):',
      type: TextInputType.number,
    );
    ...
    late double igv;
    try {
      igv = double.parse(igvString);
    } catch (e) {
      alert(context, 'Ocurrió un error\nNúmero no válido');
      return;
    }
    if (0 > igv || igv > 100) {
      alert(context, 'El IGV en % debe de estar entre 1 y 100');
      return;
    }
  ...
  void _definirDescuentoGeneral() async {
    if ((await confirm(
          context,
          '¿Definir un % de descuento general para todos los productos?',
        )) !=
        true)
      return;
    String? discountString = await prompt(
      context,
      text: '% de descuento general:',
      type: TextInputType.number,
    );
    if (discountString == null) return;
    late double discount;
    try {
      discount = double.parse(discountString);
    } catch (e) {
      alert(context, 'Ocurrió un error\nNúmero no válido');
      return;
    }
    if (0 > discount || discount > 100) {
      alert(context, 'El % debe de estar entre 1 y 100');
      return;
    }
    ...
    child: Row(
      children: [
        InkWell(
          onTap: removeUnit,
          child: Icon(
            units == 1 ? Icons.close : Icons.remove,
            color: Colors.red,
            size: 33,
          ),
        ),

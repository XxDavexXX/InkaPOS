...
class _ImprimirCierreDeTurnoState extends State<ImprimirCierreDeTurno> {
  ...
  void _print() async {
    bool? ans = await confirm(context, '¿Imprimir este cierre?');
    if (ans != true) return;
  ...
  void dispose() {
    // Seguridad adicional: solo cierra si aún hay turno activo
      ...
      setState(() {
        ...
        List<Map> registros =
            getAllRegistrosDeVenta()
                .where((x) => x['turno'].toString() == turno['id'].toString())
                .toList();

        registros.forEach(
          (reg) => reg['metodosDePago'].forEach((mp) {
            if (mp['divisa'] == 'PEN') _totalVentaSoles += mp['monto'];
            if (mp['divisa'] == 'USD') _totalVentaDolares += mp['monto'];
          }),
        );

        List<Map> pedidoEliminados =
            getAllPedidosBorrados()
                .where((Map ped) => ped['turno'] == turno['id'].toString())
                .toList();
        _pedidosEliminadosDeTipoBoleta =
            pedidoEliminados.where((x) => x['tipo'] == 'boleta').length;
        pedidoEliminados.where((x) => x['tipo'] == 'boleta').forEach((
          Map pedido,
        ) {
          ...
          pedido['productos'].forEach(
            (Map prod) =>
                totalDelPedido =
                    totalDelPedido + (prod['cantidad'] * prod['precioUnit']),
          );
          _pedidosEliminadosDeTipoBoletaSuma += totalDelPedido;
        });
        _pedidosEliminadosDeTipoFactura =
            pedidoEliminados.where((x) => x['tipo'] == 'factura').length;
        pedidoEliminados.where((x) => x['tipo'] == 'factura').forEach((
          Map pedido,
        ) {
          double totalDelPedido = 0.0;
          pedido['productos'].forEach(
        ...
        registros.forEach(
          (Map reg) => reg['metodosDePago'].forEach((Map mp) {
            switch (mp['tipo']) {
              case 'efectivo':
                if (mp['divisa'] == 'PEN')
                  pagosEnEfectivoDeVentasEnSoles += mp['monto'];
                if (mp['divisa'] == 'USD')
                  pagosEnEfectivoDeVentasEnDolares += mp['monto'];
                break;
              case 'electrónico':
                if (mp['divisa'] == 'PEN')
                  pagosElectronicosDeVentasEnSoles += mp['monto'];
                if (mp['divisa'] == 'USD')
                  pagosElectronicosDeVentasEnDolares += mp['monto'];
                break;
              case 'tarjeta':
                if (mp['divisa'] == 'PEN')
                  pagosConTarjetaDeVentasEnSoles += mp['monto'];
                if (mp['divisa'] == 'USD')
                  pagosConTarjetaDeVentasEnDolares += mp['monto'];
                break;
            }
          }),
        );
        _resumenPagosEnEfectivo = [
          {
            'dato': 'Ventas',
            'soles': pagosEnEfectivoDeVentasEnSoles,
            'dolares': pagosEnEfectivoDeVentasEnDolares,
          },
          {
            'dato': 'Propinas',
            'soles': pagosEnEfectivoDePropinasEnSoles,
            'dolares': pagosEnEfectivoDePropinasEnDolares,
          },
          {
            'dato': 'Total',
            'soles':
                pagosEnEfectivoDeVentasEnSoles +
                pagosEnEfectivoDePropinasEnSoles,
            'dolares':
                pagosEnEfectivoDeVentasEnDolares +
                pagosEnEfectivoDePropinasEnDolares,
          },
        ]; // (fields: dato, soles, dolares)
        _resumenPagosConTarjeta = [
          {
            'dato': 'Ventas',
            'soles': pagosConTarjetaDeVentasEnSoles,
            'dolares': pagosConTarjetaDeVentasEnDolares,
          },
          {
            'dato': 'Propinas',
            'soles': pagosConTarjetaDePropinasEnSoles,
            'dolares': pagosConTarjetaDePropinasEnDolares,
          },
          {
            'dato': 'Total',
            'soles':
                pagosConTarjetaDeVentasEnSoles +
                pagosConTarjetaDePropinasEnSoles,
            'dolares':
                pagosConTarjetaDeVentasEnDolares +
                pagosConTarjetaDePropinasEnDolares,
          },
        ]; // (fields: dato, soles, dolares)
        _resumenPagosElectronicos = [
          {
            'dato': 'Ventas',
            'soles': pagosElectronicosDeVentasEnSoles,
            'dolares': pagosElectronicosDeVentasEnDolares,
          },
          {
            'dato': 'Propinas',
            'soles': pagosElectronicosDePropinasEnSoles,
            'dolares': pagosElectronicosDePropinasEnDolares,
          },
          {
            'dato': 'Total',
            'soles':
                pagosElectronicosDeVentasEnSoles +
                pagosElectronicosDePropinasEnSoles,
            'dolares':
                pagosElectronicosDeVentasEnDolares +
                pagosElectronicosDePropinasEnDolares,
          },
        ]; // (fields: dato, soles, dolares)
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          //   final turnoActivo = getTurnoActual();
          //   Navigator.pop(context, turnoActivo == null);
          // }),
          MyIcon(Icons.arrow_back, () {
            cerrarTurnoActual();
            Navigator.pop(context, true);
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            Te('TOTALES:'),
                            Te(
                              '${_pedidosEliminadosDeTipoBoleta + _pedidosEliminadosDeTipoFactura}',
                            ),
                            Te(
                              '${_pedidosEliminadosDeTipoBoletaSuma + _pedidosEliminadosDeTipoFacturaSuma}',
                            ),
                          ],
                        ),
                        sep,
                        ...
                        MyTable([
                          ...
                          ..._resumenPagosEnEfectivo.map<List<Cell>>(
                            (Map data) => [
                              Cell(data['dato']),
                        ...
                        MyTable([
                          ...
                          ..._resumenPagosConTarjeta.map<List<Cell>>(
                            (Map data) => [
                              Cell(data['dato']),
                              Cell(data['soles'], width: 66),
                              Cell(data['dolares'], width: 66),
                        ...
                        MyTable([
                          ...
                          ..._resumenPagosElectronicos.map<List<Cell>>(
                            (Map data) => [
                              Cell(data['dato']),
                              Cell(data['soles'], width: 66),
                              Cell(data['dolares'], width: 66),
                        ...
                        MyTable([
                          ...
                          ..._resumenOtrosPagos.map<List<Cell>>(
                            (Map data) => [
                              Cell(data['dato']),
                              Cell(data['soles'], width: 66),
                              Cell(data['dolares'], width: 66),
                        ...
                        MyTable([
                          ...
                          ..._resumenVentaSoles.map<List<Cell>>(
                            (Map data) => [
                              Cell(data['dato']),
                              Cell(data['soles'], width: 66),
                            ],
                        ...
                        MyTable([
                          ...
                          ..._resumenVentaDolares.map<List<Cell>>(
                            (Map data) => [
                              Cell(data['dato']),
                              Cell(data['dolares'], width: 66),
                            ],
